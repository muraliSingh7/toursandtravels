...
export class OneWayAndRoundTripHandler {
  ...
  main() {
    ...
    this.createElementForFilterParameters(
      this.source,
      this.destination,
      this.flightData
    );
  ...
  createFlightDataContainerForViewer() {
    ...
    document.body
      .querySelector("[name=flightDataViewContainer]")
      .appendChild(resultViewContainer);
  ...
  filterPanelCreation() {
    ...
    document
      .querySelector("[name=tripFlightDataView]")
      .appendChild(filterPanel);
  }
  ...
  async createElementForFilterParameters(source, destination) {
    ...
    if (this.triptype == "Round-Trip") {
      ...
      name = [
        "numberOfStopsFromSourceFromTrip0",
        "numberOfStopsFromSourceFromTrip1",
        ...
        if (this.triptype == "Round-Trip") {
          switch (name[i]) {
            ...
            case name[1]:
              ...
              break;
            case name[2]:
              this.filterByArrivalAndDepartureTimeInterval(
                elementContainer,
                "departure",
                0,
                name[2],
            ...
            case name[5]:
              ...
              break;
            case name[6]:
              this.filterByAirline(elementContainer, filterResult[name[6]]);
              break;
          }
        } else {
          switch (name[i]) {
            ...
            case name[2]:
              ...
              break;
            case name[3]:
              this.filterByAirline(elementContainer, filterResult[name[3]]);
              break;
          }
        }

        document
          .querySelector("[name=filterPanel]")
  ...
  async getKeyStatisticsOfEachFilter(tripCount) {
    ...
    for (let tripNumber = 0; tripNumber < tripCount; tripNumber++) {
      filterResult["numberOfStopsFromSourceFromTrip" + tripNumber] =
        await this.filter.getFlightCountAndMinimumPriceByStoppages(tripNumber);
      ...
      for (let i = 0; i < time.length - 1; i++) {
        filterResult["departureFromSourceAsPerTimeFromTrip" + tripNumber][
          time[i] + "_" + time[i + 1]
        ] = await this.filter.getMinimumPriceOfFlightAsPerTime(
          tripNumber,
          "departure",
          time[i],
          time[i + 1]
  ...
  filterByNumberOfStops(parentElement, filterName, filterResult) {
    Object.keys(filterResult).forEach((stopNumber) => {
      ...
      checkBoxAndLabelCombinedContainer.setAttribute(
        "class",
        " checkBoxAndLabelCombinedContainer"
      );
      ...
      if (stopNumber == 0) {
        labelForStop.textContent =
          "Non-Stop (" + filterResult[stopNumber]["numberOfFlights"] + ")";
      } else {
        labelForStop.textContent =
          stopNumber +
          " Stop " +
          "(" +
          filterResult[stopNumber]["numberOfFlights"] +
          ")";
      }
      ...
      minimumPriceForStop.textContent =
        filterResult[stopNumber]["minimumPrice"];
      ...
      individualStopContainer.addEventListener("click", () => {
        ...
        if (
          !this.filterAndSortParametersSelectedByViewer.hasOwnProperty(
            filterName
          )
        ) {
        ...
        if (checkBoxForStop.checked) {
          this.filterAndSortParametersSelectedByViewer[filterName].push(
            stopNumber
          );
        } else {
          if (
            this.filterAndSortParametersSelectedByViewer[filterName].length >
              0 &&
            this.filterAndSortParametersSelectedByViewer[filterName].indexOf(
              stopNumber
            ) > -1
          ) {
            this.filterAndSortParametersSelectedByViewer[filterName].splice(
              this.filterAndSortParametersSelectedByViewer[filterName].indexOf(
                stopNumber
              ),
              1
  ...
  filterByArrivalAndDepartureTimeInterval(
    parentElement,
    departureOrArrival,
    tripNumber,
    filterName,
    resultAsPerTimeRange
  ) {
    var timing = ["0AM-6AM", "6AM-12PM", "12PM-6PM", "6PM-12AM"];
    var timeHeader = ["Before 6AM", "6AM-12PM", "12PM-6PM", "After 6PM"];
    var time = [0, 21600, 43200, 64800, 86400];
    var imgSource = [
      "https://imgak.mmtcdn.com/flights/assets/media/dt/listing/left-filters/morning_inactive.png?v=1",
    ...
    for (let i = 0; i < timing.length; i++) {
      if (resultAsPerTimeRange[time[i] + "_" + time[i + 1]]) {
        ...
        timeIntervalButton.setAttribute(
          "class",
          "timeIntervalButtonBeforeClicked"
        );
        ...
        timeIntervalMinimumPrice.textContent =
          resultAsPerTimeRange[time[i] + "_" + time[i + 1]];
        ...
        timeIntervalButton.addEventListener("click", () => {
          ...
          if (
            timeIntervalButton.classList.contains(
              "timeIntervalButtonBeforeClicked"
            )
          ) {
            timeIntervalButton.classList.remove(
              "timeIntervalButtonBeforeClicked"
            );
            timeIntervalButton.classList.add("timeIntervalButtonAfterClicked");
            this.filterAndSortParametersSelectedByViewer[
              departureOrArrival + tripNumber
            ].push([time[i], time[i + 1]]);
            console.log(this.filterAndSortParametersSelectedByViewer);
          } else if (
            timeIntervalButton.classList.contains(
              "timeIntervalButtonAfterClicked"
            )
          ) {
            timeIntervalButton.classList.remove(
              "timeIntervalButtonAfterClicked"
            ...
            if (
              ...
              let index = getIndexFromNestedArray(
                this.filterAndSortParametersSelectedByViewer[
                  departureOrArrival + tripNumber
                ],
                [time[i], time[i + 1]]
  ...
  filterByAirline(parentELement, filterResult) {
    Object.keys(filterResult).forEach((airlineCode) => {
      ...
      labelForAirline.textContent =
        airlineCode + "(" + filterResult[airlineCode]["numberOfFlights"] + ")";
      ...
      airlineMinimumPrice.textContent =
        filterResult[airlineCode]["minimumPrice"];
      ...
      individualAirlineContainer.addEventListener("click", () => {
        ...
        if (checkboxForAirline.checked) {
          this.filterAndSortParametersSelectedByViewer["airline"].push(
            airlineCode
          );
        } else {
          if (
            this.filterAndSortParametersSelectedByViewer["airline"].length >
              0 &&
            this.filterAndSortParametersSelectedByViewer["airline"].indexOf(
              airlineCode
            ) > -1
          ) {
            this.filterAndSortParametersSelectedByViewer["airline"].splice(
              this.filterAndSortParametersSelectedByViewer["airline"].indexOf(
                airlineCode
              ),
              1
  ...
  async responseOfFilterAndSortParametersSelectedByViewer() {
    const response = await this.filter.filtering(
      this.filterAndSortParametersSelectedByViewer
    );
    return response;
  }
  ...
  createElementForSortParameters() {
    ...
    if (this.triptype == "Round-Trip") {
      ...
      parameters = [
        "Sort By: ",
        `Departure_Time_From_${this.source}`,
        `Arrival_Time_To_${this.destination}`,
        `Duration_From_${this.source}`,
        `Departure_Time_From_${this.destination}`,
        `Arrival_Time_To_${this.source}`,
        `Duration_From_${this.destination}`,
        "Price",
      ];
    ...
    for (let i = 0; i < parameters.length; i++) {
      ...
      if (parameters[i] !== "Sort By: ") {
        paramElement.addEventListener("click", () => {
            ...
            if (elementName != "Sort By: ") {
              ...
              if (
                ...
                element.classList.contains(
                  "roundWayIndividualSortELementAfterClicked"
                )
              ) {
                element.classList.add("roundWayIndividualSortELement");
              } else if (
                this.triptype != "Round-Trip" &&
                element.classList.contains("individualSortELementAfterClicked")
              ) {
          ...
          if (
            !this.filterAndSortParametersSelectedByViewer.hasOwnProperty("sort")
          ) {
          ...
          if (direction == 1) {
            ...
            this.filterAndSortParametersSelectedByViewer["sort"].push(
              name[i] + 0
            ); //0 for descending
          } else {
            this.filterAndSortParametersSelectedByViewer["sort"] = [];
          ...
          if (
            this.triptype == "Round-Trip" &&
            paramElement.classList.contains("roundWayIndividualSortELement")
          ) {
            paramElement.classList.add(
              "roundWayIndividualSortELementAfterClicked"
            );
          } else if (
            this.triptype != "Round-Trip" &&
            paramElement.classList.contains("individualSortELement")
          ) {
    ...
    document
      .querySelector("[name=sortPanel]")
      .appendChild(sortingHeaderContainer);
  }
  ...
  display(data, numberOfLegs) {
    ...
    for (let i = 0; i < Math.min(data.length, 50); i++) {
      ...
      card.shadowRoot.querySelector("[name=carrier-name]").textContent =
        data[i]["airline"];
      for (let j = 0; j < numberOfLegs; j++) {
        card.shadowRoot.querySelectorAll("[name=depart-time]")[j].textContent =
          data[i]["departureFromSourceAsPerTimeFromTrip" + j];
        card.shadowRoot.querySelectorAll('[name="depart-place"]')[
          j
        ].textContent = data[i]["sourceFromTrip" + j];
        card.shadowRoot.querySelectorAll("[name=duration]")[j].textContent =
          data[i]["durationFromTrip" + j];
        // card.shadowRoot.querySelectorAll("[name=stoppage]")[j].textContent = data[i]['numberOfStopsFromSourceFromTrip' + j];
        this.drawCanvasForNumberOfStoppage(
          card.shadowRoot.querySelectorAll("[name=stoppage]")[j],
          data[i]["numberOfStopsFromSourceFromTrip" + j]
        );
        if (data[i]["numberOfStopsFromSourceFromTrip" + j] == 0) {
          card.shadowRoot.querySelectorAll("[name=numberOfStoppage]")[
            j
          ].textContent = "Non-Stop";
        } else {
          card.shadowRoot.querySelectorAll("[name=numberOfStoppage]")[
            j
          ].textContent =
            data[i]["numberOfStopsFromSourceFromTrip" + j] + " Stop";
        }
        card.shadowRoot.querySelectorAll("[name=arrival-time]")[j].textContent =
          data[i]["arrivalAtDestinationAsPerTimeFromTrip" + j];
        card.shadowRoot.querySelectorAll("[name=arrival-place]")[
          j
        ].textContent = data[i]["destinationFromTrip" + j];
      }
      card.shadowRoot.querySelector("[name=price]").textContent =
        data[i]["price"];
