    ...
    form.addEventListener("submit", async function (event) {
        ...
        for (let i = 0; i < listOfTripInformationElement.length; i++) {
            ...
            if (i == listOfTripInformationElement.length - 1) {
                ...
                tripInformation['returndate'] = returndate.trim();
        ...
        // console.log(inputFieldNotFilled);


        if (inputFieldNotFilled) {
            displayErrorInViewArea();
        } else {
            clearViewArea();
            localStorage.clear();
            if (tripType == "One-Way") {
...
function displayErrorInViewArea() {
    let flightDataViewContainer = document.body.querySelector('[name=flightDataViewContainer]');
    flightDataViewContainer.textContent = "Please fill in all the required fields";
