...
export class FilterCache {
  constructor(tripNumber, results) {
    //tripCount,results
    results = results || [];
    this.init(tripNumber, results); //tripCount,results
  }

  async init(tripNumber, results) {
    //tripCount,results
  ...
  async getFlightCountAndMinimumPriceByStoppages(tripNumber) {
    ...
    filterParameters[0]["numberOfStopsFromSourceFromTrip" + tripNumber] = {
      $gte: 0,
    };
    ...
    let result = await this.databaseForTrip.find(
      { $and: filterParameters },
      { $orderBy: sortParameters }
    );
    console.log(result);
    // result.forEach((data)=>{
    //     console.log(data);
    ...
    if (result.length > 1) {
      let minimumStoppages =
        result[0]["numberOfStopsFromSourceFromTrip" + tripNumber];
      let maximumStoppages =
        result[result.length - 1][
          "numberOfStopsFromSourceFromTrip" + tripNumber
        ];
      // console.log(min+" "+max);
      ...
      for (let i = minimumStoppages; i <= maximumStoppages; i++) {
        ...
        let filterResult = await this.databaseForTrip.find(
          { $and: filterParameters },
          { $orderBy: sortParameters }
        );
        ...
        stoppageDetails[i] = {
          numberOfFlights: filterResult.length - 1,
          minimumPrice: filterResult[0]["price"],
        };
      }
    }
  ...
  async getMinimumPriceOfFlightAsPerTime(
    tripNumber,
    arrivalOrDestination,
    minimumTimeInSeconds,
    ...
    if (arrivalOrDestination == "arrival") {
      filterParameters[0][
        "arrivalAtDestinationAsPerTimeInSecondsFromTrip" + tripNumber
      ] = { $gte: minimumTimeInSeconds, $lt: maximumTimeInSeconds };
      sortParameters[
        "arrivalAtDestinationAsPerTimeInSecondsFromTrip" + tripNumber
      ] = 1;
    } else {
      filterParameters[0][
        "departureFromSourceAsPerTimeInSecondsFromTrip" + tripNumber
      ] = { $gte: minimumTimeInSeconds, $lt: maximumTimeInSeconds };
      sortParameters[
        "departureFromSourceAsPerTimeInSecondsFromTrip" + tripNumber
      ] = 1;
    ...
    let flightsSortedByPrice = await this.databaseForTrip.find(
      { $and: filterParameters },
      { $orderBy: sortParameters }
    );
    // console.log(result);
  ...
  async getFlightStatisticsByAirlines() {
    let airlineStats = {};
    let uniqueAirlines = await this.databaseForTrip.find({
      $distinct: { airline: 1 },
    });
    ...
    uniqueAirlines.forEach(async (airlineInformation) => {
      ...
      let filterParameters = [
        { airline: { $eq: airlineInformation.airline } },
        { price: { $gt: 0 } },
      ];
      let sortParameters = { airline: 1, price: 1 };

      let flightsSortedByPriceForAirline = await this.databaseForTrip.find(
        { $and: filterParameters },
        { $orderBy: sortParameters }
      );
      // console.log({ $and: filterParameters }, { $orderBy: sortParameters });
      ...
      if (flightsSortedByPriceForAirline.length > 0) {
        airlineStats[airlineInformation.airline] = {
          numberOfFlights: flightsSortedByPriceForAirline.length,
          minimumPrice: flightsSortedByPriceForAirline[0].price,
        };
      }
    });
  ...
  async filtering(filterBy) {
    ...
    Object.keys(filterBy).forEach((filter) => {
      ...
      // }
      if (
        filter.includes("numberOfStopsFromSource") ||
        filter.includes("airline")
      ) {
        let obj = {};
        ...
        if (filterBy[filter].length > 0) {
          if (filterBy[filter].length == 1) {
            if (filter.includes("departure")) {
              obj[
                "departureFromSourceAsPerTimeInSecondsFromTrip" +
                  filter.substring(filter.length - 1)
              ] = { $gte: filterBy[filter][0][0], $lt: filterBy[filter][0][1] };
            } else {
              obj[
                "arrivalAtDestinationAsPerTimeInSecondsFromTrip" +
                  filter.substring(filter.length - 1)
              ] = { $gte: filterBy[filter][0][0], $lt: filterBy[filter][0][1] };
            }
          } else {
